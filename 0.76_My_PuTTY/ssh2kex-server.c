    ...
    if (s->kex_alg->main_type == KEXTYPE_DH) {
        ...
        if (dh_is_gex(s->kex_alg)) {
            ...
            if (pktin->type != SSH2_MSG_KEX_DH_GEX_REQUEST &&
                ...
                ssh_proto_error(s->ppl.ssh, "等待 Diffie-Hellman group exchange 时，"
                                "收到意外的数据包，"
                                "类型：%d (%s)", pktin->type,
        ...
        if (pktin->type != s->kex_init_value) {
            ssh_proto_error(s->ppl.ssh, "等待 Diffie-Hellman 初始化数据包时，"
                            "收到意外的数据包，"
                            "类型：%d (%s)", pktin->type,
                            ssh2_pkt_type(s->ppl.bpp->pls->kctx,
                                          s->ppl.bpp->pls->actx,
...

        {
            ...
            if (!get_err(pktin) && !s->K) {
                ssh_proto_error(s->ppl.ssh, "在 ECDH 初始化数据包中收到"
                                "无效的椭圆曲线点");
                *aborted = true;
                return;
            }
        ...
        if (pktin->type != SSH2_MSG_KEXRSA_SECRET) {
            ssh_proto_error(s->ppl.ssh, "等待 RSA kex 机密时，"
                            "收到意外的数据包，类型：%d (%s)",
                            pktin->type,
                            ssh2_pkt_type(s->ppl.bpp->pls->kctx,
                                          s->ppl.bpp->pls->actx,
        ...
        if (!s->K) {
            ssh_proto_error(s->ppl.ssh, "无法解析 RSA kex 机密");
            *aborted = true;
            return;
        }
